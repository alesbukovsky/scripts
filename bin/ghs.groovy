...
def cli = new CliBuilder(usage:'ghs [options]')
cli.with {
    ...
    r longOpt:'repo', args:1, argName:'owner/repo', required:true, 'repository name including owner'
...
http.headers['User-Agent'] = 'ghs-script'
...
switch (opts.fs[0]) {
    ...
    case 'pull': 
        http.request(GET, JSON) { req ->
            uri.path = "/repos/${opts.r}/pulls/${opts.fs[1]}"
    ...
    case 'branch': 
        http.request(GET, JSON) { req ->
            uri.path = "/repos/${opts.r}/git/refs/heads/${opts.fs[1]}"
            response.success = { resp, data ->
    ...
    case 'commit': 
        if (opts.fs[1].size() < 40) {
            http.request(GET, TEXT) { req ->
                uri.path = "/repos/${opts.r}/commits/${opts.fs[1]}"
...
if (!opts.d) {
    ...
    http.request(POST, JSON) { req ->
        uri.path = "/repos/${opts.r}/statuses/${hash}"
    ...
    if (opts.c && opts.fs[0] == 'pull') {
        http.request(POST, JSON) { req ->
            uri.path = "/repos/${opts.r}/issues/${opts.fs[1]}/comments"
            body = [ body: opts.c ]
            response.'201' = { resp ->
